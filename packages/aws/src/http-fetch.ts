/** @fileoverview HttpClient over the Node global `fetch`, with manual-redirect + timeout + SSRF re-check (G6). @module @anyhook/aws */
...
/**
 * HttpClient backed by Node's global `fetch` (undici). Redirects are NOT followed
 * (`redirect: 'manual'`): a 3xx on a webhook target is treated as a delivery failure (§8).
 * Timeouts map to `{status:'timeout'}`, other transport errors to `{status:'network'}`.
 */
  ...
  return {
    async post(url, body, headers, timeoutMs): Promise<HttpResult> {
      ...
      try {
        ...
        // for `redirect:'manual'`. Either way, report a 3xx so the engine classifies it permanent.
